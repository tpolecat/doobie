...
object connection {
  ...
  def createStatement[A](k: StatementIO[A]): ConnectionIO[A] =
    IFC.createStatement.flatMap(
      s => WeakAsyncConnectionIO.cancelable(IFC.embed(s, k).attempt.flatTap(_ => IFC.embed(s, IFS.close)).rethrow, IFC.embed(s, IFS.close))
    )

  /** @group Statements */
  def createStatement[A](rst: ResultSetType, rsc: ResultSetConcurrency)(k: StatementIO[A]): ConnectionIO[A] =
    IFC.createStatement(rst.toInt, rsc.toInt).flatMap(
      s => WeakAsyncConnectionIO.cancelable(IFC.embed(s, k).attempt.flatTap(_ => IFC.embed(s, IFS.close)).rethrow, IFC.embed(s, IFS.close))
    )
  ...
  def createStatement[A](
    ...
    IFC.createStatement(rst.toInt, rsc.toInt, rsh.toInt).flatMap(
      s => WeakAsyncConnectionIO.cancelable(IFC.embed(s, k).attempt.flatTap(_ => IFC.embed(s, IFS.close)).rethrow, IFC.embed(s, IFS.close))
    )
