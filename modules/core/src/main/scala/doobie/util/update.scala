...
object update {
  ...
  trait Update[A] { u =>
    ...
    /** Program to execute a batch update and yield a count of affected rows. Note that failed updates are not reported
      * (see https://github.com/tpolecat/doobie/issues/706). This API is likely to change.
      * @group Execution
      */
